...
public  class PythonExecutor:IScriptExecutor
{
    public   PythonExecutor( string pythonPath)
    {
       ...
       // PythonEngine.PythonPath = AppPath.ScriptsPath + ";" + PythonEngine.PythonPath;
    ...
    public  dynamic Execute(string moduleName, string functionName, params object[] args)
    {
        using (Py.GIL())
        {
            var pyObject = Py.Import(moduleName);
          
            return pyObject.InvokeMethod(functionName, args.Select(x=>x.ToPython()).ToArray());
        }
    }
}